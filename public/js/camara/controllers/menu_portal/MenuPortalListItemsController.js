(function() {
   ...
   angular.module('SiteCamaraAdminApp').controller('MenuPortalListItemsController', MenuPortalListItemsController);

   MenuPortalListItemsController.$inject = [ '$scope', 'Utils',
                                             'MenuPortalService', 'messages',
                                             '$uibModal', '$stateParams', '$state']
   function MenuPortalListItemsController( $scope, Utils,
                                           MenuPortalService, messages,
                                           $uibModal, $stateParams, $state ) {
      var $ctrl = this;
      ...
      $ctrl.groupsToggleStatus = MenuPortalService.getGroupsToggleStatus(); //it stores the collapsed status of the nodes
      $ctrl.defaultCollapsedStatus = true; //default collapsed status

      //refresh the tree, get tha data from API
      $ctrl.refresh = function () {
         return MenuPortalService.getMenuPortalTree().then(function(result) {
            $ctrl.menuItemsTree = result.menuPortalTree;
         });
      }
      ...
      $ctrl.newSubMenuItem = function(parentMenuItem) {
         if(parentMenuItem) {
            $state.go('menuPortal.newSub', { 'parentMenuItem': parentMenuItem });
         }
      }

      $ctrl.editMenuItem = function (menuItem) {
         if(menuItem) {
            $state.go('menuPortal.edit', { 'menuItem': menuItem });
         }
      }
     ...
     $ctrl.removeMenuItem = function (scope) {
        ...
        $uibModal.open({
           ...
           App.blockUI();
           scope.remove();
           MenuPortalService.deleteDeeplyMenuItem(scope.$modelValue._id).then(function(result) {
              if(scope.$parentNodeScope) {
                 return MenuPortalService.saveMenuItem(_extractMenuItemModel(scope.$parentNodeScope));
              } else {
                 return result;
              }
     ...
     $ctrl.treeOptions = {
        ...
        dragStop : function (e) {
            ...
            if(destNodeScope) {
               ...
               MenuPortalService.saveMenuItem(destParentMenuItem)
                                 .then(function(result) {
                  if(sourceParentMenuItem) {
                     return MenuPortalService.saveMenuItem(sourceParentMenuItem);
                  } else {
                     //the source was in the root
                     menuItem.isRoot = false;
                     return MenuPortalService.saveMenuItem(menuItem);
                  }
               }).then(function(result) {
               ...
               MenuPortalService.saveMenuItem(menuItem).then(function(result) {
                  var sourceParentMenuItem = sourceNodeScope.$parentNodeScope ? _extractMenuItemModel(sourceNodeScope.$parentNodeScope) : null;
                  if(sourceParentMenuItem) {
                     return MenuPortalService.saveMenuItem(sourceParentMenuItem);
                  } else {
                  ...
                  return MenuPortalService.updateMenuItemsOrders(menuItemsOrders);
               }).then(function(result) {
                  $ctrl.setMessage( _templateMessage( messages.menuItemMoved,
                                                    { 'title': menuItem.title }) );
                  return $ctrl.refresh();
